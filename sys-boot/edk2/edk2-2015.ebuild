...
pkg_setup() {
	if use x86 ; then
		...
		export UNAME_ARCH=$( uname -m | sed s,i[3456789]86,Ia32, )
...
src_unpack() {
	...
	GCC_VERS=$(gcc-major-version)$(gcc-minor-version)
	if ! grep "(GCC${GCC_VERS}_BIN)" "${S}/BaseTools/Conf/tools_def.template"; then
